<?php 
/** SITE SETTINGS **/
$path = 'http://localhost/misc/ms/v2/';
...
/** Functions **/
function langs() {
    global $langs, $frontpath, $lang;    
    foreach ($langs as $l => $name) {
		echo "<a href='" . BASE_PATH . $l . "'><img src='{$frontpath}img/$l.png' " . ($lang == $l?" class='active'":'') . " title='$name'></a> ";
    }
}
...
=======
		//$_path = $path . $lang . ($k != '' ? '/' . $k : '');
		if($k == 'home') $v = '<i class="fa fa-home"></i>';
		echo "<a href='#$k' data-menuanchor='$k'" . ($k == 'home' ? ' class="active"':''). ">$v</a>";
